...
import { convertToPinecone } from "../services/pineconeService";
...
export const processKnowledgebaseFiles = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    await convertToPinecone(extractedTexts[0].text,`agent-${agentId}`)
...
export const getAgentNameById = async (
  req: Request,
  res: Response
): Promise<void> => {
  const id = parseInt(req.params.id, 10);
  ...
  try {
    // Fetch only the name for the given agent ID
    ...
    console.error(err);
    res.status(500).json({ status: 0, message: "Failed to fetch agent name." });
  }
};
