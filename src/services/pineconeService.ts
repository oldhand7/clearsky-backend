...
import { pdf2text, splitTextIntoChunks } from "../helpers/utils";
...
export async function get_embedding(chunk: string) {
  ...
  const embeddings = await pc.inference.embed(model, [chunk], {
    inputType: "passage",
    truncate: "END",
  });
...
export async function uploadToPinecone(documents: string[], namespace: string) {
  ...
  const batchProcessEmbeddings = async (
    documents: string[],
    batchSize: number,
  ) => {
    const embeddings = [];
  ...
  const batchUpsert = async (
    ...
    for (let i = 0; i < records.length; i += upsertBatchSize) {
      ...
      logger.info(
        `Upserting batch ${Math.floor(i / upsertBatchSize) + 1} of ${
          Math.ceil(records.length / upsertBatchSize)
        }`,
      );

      try {
        const response = await index.namespace(namespace).upsert(batch as any);
...
export async function queryPinecone(query: string, index_name: string) {
  ...
  const model = "multilingual-e5-large";
  const query_embedding = await pc.inference.embed(model, [query], {
    inputType: "passage",
    truncate: "END",
  });
...
export async function convertToPinecone(
  extractedTexts: string,
  namespace: string,
) {
  logger.info(`Agent namespace -----------------> ${namespace}`);

  logger.info(`Start splitting text into chunks...`);
  const documents = await splitTextIntoChunks(extractedTexts);
  ...
  const response = await uploadToPinecone(unique_documents, namespace);
  return response;
}
